...
class MainWindow(
    ...
    def __init__(self):
        ...
        # 记录是否需要在停止仿真后重置视口全屏状态（针对从运行时模式进入全屏的情况）
    ...
    async def init(self):
        ...
        connect(self.menu_viewport.aboutToShow, self.prepare_viewport_menu)
    ...
    async def _init_ui(self):
        ...
        self.menu_viewport = self.menu_bar.addMenu("视口")
    ...
    async def start_sim_fullscreen(self):
        await SimulationRequestBus().start_simulation()
    ...
    def prepare_viewport_menu(self):
    ...
    def prepare_run_menu(self):
        ...
        action_start_fullscreen = self.menu_run.addAction("开始模拟（全屏）")
        action_start_fullscreen.setEnabled(sim_state == SimulationState.Stopped)
        connect(action_start_fullscreen.triggered, self.start_sim_fullscreen)
    ...
    def is_viewport_fullscreen(self) -> bool:
    ...
    def full_screen_message_bubble(
        ...
        bubble = QtWidgets.QLabel(message, parent)
        ...
        x =  (parent.width() - bubble.width()) // 2
        y = 50
        bubble.move(x, y)
        bubble.show()

        QtCore.QTimer.singleShot(duration, bubble.deleteLater)
