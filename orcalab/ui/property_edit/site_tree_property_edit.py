...
# 与 Joint/Geom 高亮隔离的独立存储
_site_highlight_store: dict[str, Set[int]] = {}
...
class SiteButton(TreeLeafButton):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            highlight_store=_site_highlight_store,
        )
...
class SiteTreePropertyEdit(BasePropertyEdit):
    ...
    def __init__(
        ...
        self._leaf_buttons: dict[str, SiteButton] = {}
        self._init_ui()
        asyncio.ensure_future(self._restore_highlights())

    async def _restore_highlights(self):
        for eid in _site_highlight_store.get(self._actor_path_str, set()):
    ...
    def _init_ui(self):
        ...
        if tree_data:
            highlight_set = _site_highlight_store.setdefault(self._actor_path_str, set())
            _build_nodes(
                ...
                button_cls=SiteButton, skip_unnamed=False,
            )
