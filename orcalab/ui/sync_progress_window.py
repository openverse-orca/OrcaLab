...
class AssetItemWidget(QtWidgets.QWidget):
    ...
    def set_name_size(self, name: str, size: int):
        """更新资产包名字"""
...
class SyncProgressWindow(QtWidgets.QDialog):
    ...
    _set_name_size_signal = QtCore.Signal(str, str, int)  # asset_id, status
    ...
    def set_asset_name_size(self, asset_id: str, name: str, size: int):
        """线程安全：设置资产包状态"""
        self._set_name_size_signal.emit(asset_id, name, size)
    
    def _set_name_size_impl(self, asset_id: str, name: str, size: int):
        """内部实现：设置资产包状态"""
        if asset_id in self.asset_widgets:
