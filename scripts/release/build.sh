...
if [ "$BUILD_MODE" = "test" ]; then
    ...
    if [ -f "$CONFIG_FILE" ]; then
        ...
        if [[ "$OSTYPE" == "darwin"* ]]; then
            # macOS
            sed -i '' 's|https://simassets.orca3d.cn/api| https://simassetest.orca3d.cn/api|g' "$CONFIG_FILE"
            sed -i '' 's|https://simassets.orca3d.cn/|https://simassetest.orca3d.cn/|g' "$CONFIG_FILE"
        else
            # Linux
            sed -i 's|https://simassets.orca3d.cn/api| https://simassetest.orca3d.cn/api|g' "$CONFIG_FILE"
            sed -i 's|https://simassets.orca3d.cn/|https://simassetest.orca3d.cn/|g' "$CONFIG_FILE"
