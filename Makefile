# Makefile for OrcaLab
# Provides convenient shortcuts for common tasks

.PHONY: help clean build check test-install release-test release-prod bump-version setup-pypirc check-pypirc

help:  ## Show this help message
	@echo "OrcaLab - Development Commands"
	@echo ""
...
setup-pypirc:  ## Setup PyPI configuration file
	@./scripts/release/setup_pypirc.sh

check-pypirc:  ## Check PyPI configuration
	@./scripts/release/check_pypirc.sh
...
bump-version:  ## Bump version (usage: make bump-version VERSION=25.9.1)
	@if [ -z "$(VERSION)" ]; then \
		echo "Error: VERSION is required. Usage: make bump-version VERSION=25.9.1"; \
		exit 1; \
	fi
...
test:  ## Run tests (if available)
	@echo "Running tests..."
	pytest test/ || echo "No tests found"
